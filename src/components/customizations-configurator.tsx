...
export default function CustomizationsConfigurator({
  trigger,
  orderItem,
  menuItem: { _id: menuItemId, name, description, price, customizations },
}: {
  trigger: ReactNode
  orderItem?: Pick<ToImmutable<LiveOrderItem>, 'id' | 'customizations'>
  menuItem: Pick<
    Doc<'menuItems'>,
  ...
  const handleUpdateCustomizations = useCustomizationsMutation()

  const initialCustomizations = Object.fromEntries(
    orderItem
      ? orderItem.customizations
          .entries()
          .map(([group, customization]) => [group, [...customization]])
          ...
          >
            {orderItem ? (
              ...
              <Button
                ...
                disabled={!myOwnerId || hasInvalidCustomization}
              >
